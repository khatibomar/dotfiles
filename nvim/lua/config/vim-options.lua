...
-- Add a general autocommand for formatting on save
api.nvim_create_autocmd("BufWritePre", {
  group = group,
  callback = function()
    -- Use LSP formatting if available, otherwise use built-in formatting
    ...
    if has_formatter then
      ...
      -- This is used for plugin contextindent
      print("Formatted with built-in")
      local curpos = vim.fn.getpos(".")     -- Save cursor position
      local winsaveview = vim.fn.winsaveview() -- Save screen view
      vim.cmd("keepjumps normal! gg=G")     -- Run formatting
      vim.fn.setpos(".", curpos)            -- Restore cursor position
      vim.fn.winrestview(winsaveview)       -- Restore screen view
    end
  end,
